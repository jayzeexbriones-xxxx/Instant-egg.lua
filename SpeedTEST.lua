...
-- ============================================
-- FORCE GRAB — multiple methods
-- ============================================
function utility:forceGrab(egg)
    local char = self.LocalPlayer.Character
    ...
    -- Method 1: remote
    pcall(function()
        if self.EggState and self.EggState.CarryFieldEgg then
            self.EggState.CarryFieldEgg(egg.Uid)
        end
    end)
    task.wait(0.3)
    ...
    -- Find model
    local slots = self.Workspace:FindFirstChild("AreaEggSlotsClient", true)
    local model = slots and slots:FindFirstChild(egg.Uid) or self.Workspace:FindFirstChild(egg.Uid, true)

    -- Method 2: TP closer then fire
    if model then
        local closePos = egg.BoundsCFrame.Position + (hrp.Position - egg.BoundsCFrame.Position).Unit * 2
        pcall(function() hrp.CFrame = CFrame.new(closePos) end)
        task.wait(0.2)
...
-- ============================================
-- STEP 6: AUTO EGG (Forest → Best → Stay)
-- ============================================
    ...
    self.eggConn = task.spawn(function()
        while self.eggEnabled do
            pcall(function()
                ...
                local grabbed = self:forceGrab(baitEgg)
                ...
                local waitTime = 0
                while waitTime < 8 and not pecked do
                    task.wait(0.1)
...
local function createUI()
    ...
    local eggToggle = makeToggle(95, "Auto Forest → Best", "🥚")
