-- ============================================================
-- 🕊️ FLY HACK — STEADY 4 STUDs/SECOND
-- ============================================================
...
function FlyState.start()
    ...
    FlyState.conn = RunService.Heartbeat:Connect(function()
        ...
        if UserInputService:IsKeyDown(FlyState.keys.forward) then
            moveDir = moveDir + lookDir
        end
        if UserInputService:IsKeyDown(FlyState.keys.back) then
            moveDir = moveDir - lookDir
        end
        ...
        -- 🎯 STEADY: 4 studs per SECOND (hindi per frame)
        -- BodyVelocity.Velocity = studs/second
        local velocity = (moveDir * FlyState.speed) 
            + Vector3.new(0, vertical * FlyState.speed, 0)

        if FlyState.bodyVelocity then
            FlyState.bodyVelocity.Velocity = velocity
        end
    ...
    print("[FLY] ✅ Fly ENABLED — 4 studs/sec (steady)")
end

function FlyState.stop()
    if not FlyState.enabled then return end
    FlyState.enabled = false
...
ui.btn.MouseButton1Click:Connect(function()
    if not FlyState.enabled then
        setToggle(true)
        ui.Status.Text = "🕊️ Flying (4 studs/s)..."
